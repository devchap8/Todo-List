...
const toggleTaskChecked = (event) => {
    if(event.target.classList.contains("taskCheck")) {
        ...
        let taskInList;
        for(let listTask of TaskState.getTaskList()) {
        ...
        if(event.target.checked) {
            ...
            if(taskInList.checked === false) TaskState.toggleChecked(taskInList);
        }
        else if(!(event.target.checked)) {
            task.classList.remove("checked");
            if(taskInList.checked === true) TaskState.toggleChecked(taskInList);
        }
    }
    TasksLocalStorage.storeList("taskList", TaskState.getTaskList());
}
...
const setupInitialTasksProjects = () => {
    ...
    TaskState.makeTask("Debug Program", "09/16/2026", "This is the description", 2, "Project 1");
    TaskState.makeTask("Pay Car Insurance", "08/16/2026", "This is the description", 3, "Project 2");
    TaskState.makeTask("Go Outside", "09/16/2027", "This is the description", 4, "Project 1");
    TaskState.makeTask("Code ToDo List", "09/25/2026", "This is the description", 5, "Project 1");
    const today = format(new Date(), 'MM/dd/yyyy');
    TaskState.makeTask("Due Today", today, "This is the description", 5, "Project 1");
    TaskState.toggleChecked(TaskState.getTaskList()[4])
...
const checkTasksInDom = () => {
    for(let task of TaskState.getTaskList()) {
        if(task.checked) {
            const taskInDom = document.getElementById(task.id);
            taskInDom.classList.add("checked");
            const taskCheck = taskInDom.querySelector(".taskCheck");
